...
| - Disease progression (S1 to S2) in the age-dependent model |$Beta$      | \(\alpha\) = 84, \(\beta\) = 716    | 0.105|   0.01  |
 ...
 - Weibull shape parameter (S1 to S2) in the model with tunnel states Annual mortality |$Lognormal$ | $log(\mu)$ = $log(1.1)$  ,  $log(\sigma)$ = 0.02      | 1.1  |   0.02  |
 - Hazard ratio of death in S1 vs H |$Lognormal$ | $log(\mu)$ = $log(3)$    ,  $log(\sigma)$ = 0.01      | 3    |   0.03  |
 - Hazard ratio of death in S2 vs H |$Lognormal$ | $log(\mu)$ = $log(10)$  ,  $log(\sigma)$ = 0.2        | 10   |   1     |
 - Log odds ratio of S1 to S2 |$Normal$    | $\mu$ = $log(0.6)$      ,  $\sigma$ = 0.1             | -0.51|   0.1   |
 | Annual costs                                                 |            |                                    |      |         |
 - Healthy individuals                                        |$Gamma$     | \(\mathbf{shape}\) = 100  , \(\mathbf{Scale}\) = 20   | 2000 |   200   |
 - Sick individuals in S1                                     |$Gamma$     | \(\mathbf{shape}\) = 177.8, \(\mathbf{Scale}\) = 22.5 | 4000 |   300   |
 - Sick individuals in S2                                     |$Gamma$     | \(\mathbf{shape}\) = 225  , \(\mathbf{Scale}\) = 66.7 | 15000|   1000  |
 - Cost of treatment A for individuals in S1 or S2            |$Gamma$     | \(\mathbf{shape}\) = 576  , \(\mathbf{Scale}\) = 20.8 | 12000|   500   |
 - Cost of treatment B for individuals in S1 or S2            |$Gamma$     | \(\mathbf{shape}\) = 676  , \(\mathbf{Scale}\) = 19.2 | 13000|   500   |
 | Utility weights                                            |            |                                    |      |         |
 ...
 | Intervention effect                                        |            |                                    |      |         |
 | - Utility for treated individuals in S1 Transition rewards |$Beta$      | \(\alpha\) = 300, \(\beta\) = 15   | 0.95 |   0.012 |
