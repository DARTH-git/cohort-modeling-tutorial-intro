...
The variable `v_m_init` is used to initialize $M$ represented by `m_M` for the cohort under under strategies SoC or A, and by `m_M_trtB` for the cohort under strategies B or AB.
...
Note that the initial state vector, `v_m_init`, can be modified to account for the cohort's distribution across the states at the start of the simulation and might vary by strategy.
